...
function JobsTable(props) {
	...
	useEffect(() => {
		...
		ApiService.getCustomerJobs(user.customerid).then(response => {
			if (isMountedRef.current) {
				setData(FuseUtils.filterArrayByString(response.data, search));
			}
		});

		return () => isMountedRef.current = false;

	}, [user.customerid, props]);
	...
	function handleClick(item) {
		props.history.push(`/apps/client/job/${item.jobid}`);
	}
