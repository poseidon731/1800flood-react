...
function Chat(props) {
	...
	const selectedChannelId = useSelector(({ chatApp }) => chatApp.selectedChannelId);
	...
	function shouldShowContactAvatar(item, i) {
		return (
			item.userid === selectedChannelId &&
			((chat.messages[i + 1] && chat.messages[i + 1].userid !== selectedChannelId) || !chat.dialog[i + 1])
		);
	}

	function isFirstMessageOfGroup(item, i) {
		return i === 0 || (chat.messages[i - 1] && chat.messages[i - 1].userid !== item.userid);
	}

	function isLastMessageOfGroup(item, i) {
		return i === chat.messages.length - 1 || (chat.messages[i + 1] && chat.messages[i + 1].userid !== item.userid);
	}
	...
	function onMessageSubmit(ev) {
		...
		dispatch(Actions.sendMessage(messageText, chat.channelid, user.id)).then(() => {
			setMessageText('');
		});
	}
			...
			<FuseScrollbars ref={chatRef} className="flex flex-1 flex-col overflow-y-auto">
				{chat && chat.length > 0 ? (
					<div className="flex flex-col pt-16 px-16 ltr:pl-56 rtl:pr-56 pb-40">
						{chat.map((item, i) => {
							const contact = item.userid === user.userid ? user : item;
							return (
								<div
									...
									className={clsx(
										...
										{ me: item.userid === user.userid },
										{ contact: item.userid !== user.userid },
